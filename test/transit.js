...
function renderMap()
{
	...
	google.maps.event.addListener(marker, 'click', function() {
		if (success) {
			infowindow.setContent(marker.title+" The closest station is "+closestStationName+", and it is approximately "+closestStationDistance.toString()+" miles away from you.");
		} else {
			infowindow.setContent(marker.title+" We couldn't locate the closest station due to an API error.")
		}
...
function plotStations()
{
	...
	for (var index in stations) {
		if (scheduleJSON["line"].toUpperCase() == stations[index]["Line"].toUpperCase()) {
			pt = new google.maps.LatLng(stations[index]["Lat"], stations[index]["Long"]);
			marker = new google.maps.Marker({
				position: pt,
				title: stations[index]["Station"],
				icon: tico,
				visible: true
			});
			marker.setMap(map);
			stationCoordinates.push(pt);

			google.maps.event.addListener(marker, 'click', function(idx) {
				return function() {
					//alert(idx);
					infowindow.close();
					infowindow.setContent(stations[idx]["Station"]);
					infowindow.open(map, this);
				}
			} (index)
			);

			checkIfClosest(pt.k, pt.A, stations[index]["Station"]);
		}
	}
	//makePolyLine();
}

function checkIfClosest(station_lat, station_long, station_name)
{
	var dist = haversineFormula(me.k, me.A, station_lat, station_long);
	if (dist < closestStationDistance) {
		closestStationDistance = dist;
		closestStationName = station_name;
	}
}
